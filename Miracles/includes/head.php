...
if(($GLOBALS['miraclesIfCompressHTML'] ?? 'off') == 'on') ob_start(); //ob 截取开始 ?>
<!DOCTYPE HTML>
<html<?php if($this->options->grayTheme && $this->options->grayTheme==1): ?> class="html-filter-gray"<?php endif; ?>>
  <head>
    ...
    <meta name="description" content="<?php if($description != '') { echo htmlspecialchars($description, ENT_QUOTES, 'UTF-8'); } elseif($this->is('post') || $this->is('page')) { $this->excerpt(50); } else { Helper::options()->description(); } ?>" />
    <meta property="og:title" content="<?php Contents::title($this); ?>" />
    <meta property="og:description" content="<?php if($description != '') { echo htmlspecialchars($description, ENT_QUOTES, 'UTF-8'); } elseif($this->is('post') || $this->is('page')) { $this->excerpt(50); } else { Helper::options()->description(); } ?>" />
    ...
    <meta property="og:url" content="<?php if($this->is('post') || $this->is('page')) { $this->permalink(); } else { echo rtrim($this->options->siteUrl(), '/') . '/' . ltrim($this->request->getRequestUrl(), '/'); } ?>" />
    ...
    <meta name="twitter:description" content="<?php if($description != '') { echo htmlspecialchars($description, ENT_QUOTES, 'UTF-8'); } elseif($this->is('post') || $this->is('page')) { $this->excerpt(50); } else { Helper::options()->description(); } ?>" />
