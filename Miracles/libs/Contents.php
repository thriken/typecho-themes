...
class Contents
{
    ...
    /**
     * 轻量级摘要解析器（仅文本处理，不涉及图片/表情/DB查询）
     */
    static public function parseExcerpt($data, $widget, $last){
        $text = empty($last) ? $data : $last;
        if ($widget instanceof Widget_Archive) {
            $text = Contents::parseTextColor(Contents::parseRuby(Contents::parseTip($text)));
        }
        return $text;
    }
    ...
    static public function parseContent($data, $widget, $last){
        $db=Typecho_Db::get();
        $_theme_opts = $db->fetchAll($db->select('value')->from('table.options')->where('name = %s', "theme:Miracles")->limit(1));
        // 安全提取 loading_image，防止主题设置未保存或格式异常
        $load_image = 'block';
        if (!empty($_theme_opts[0]["value"]) && strpos($_theme_opts[0]["value"], '"loading_image"') !== false) {
            $_parts = explode('"loading_image";', $_theme_opts[0]["value"], 2);
            if (isset($_parts[1])) {
                $_parts2 = explode('"', $_parts[1], 2);
                if (isset($_parts2[1])) {
                    $_parts3 = explode('";', $_parts2[1], 2);
                    if (isset($_parts3[0])) {
                        $load_image = $_parts3[0];
                    }
                }
            }
        }
        $text = empty($last) ? $data : $last;
        if ($widget instanceof Widget_Archive) {
            // 确保全局变量已初始化（themeInit 可能未执行，如后台预览）
            $_cdn = $GLOBALS['miraclesOptions_CDN'] ?? '0';
            $_load = $GLOBALS['miraclesOptions_loading_image'] ?? 'block';
            //ParseOther
            ...
            //LazyLoad
            $text = preg_replace('/<img (.*?)src(.*?)(\/)?>/','<img $1src="'.Utils::addLoadingImages($_cdn, $_load, 'normal').'" data-gisrc$2 data-gazeimg />',$text);
			//owo
        ...
        $rp = '<div class="col-lg-2 col-6 col-md-3 links-container">
		    <a href="${2}" title="${4}" target="_blank" class="links-link">
			  <div class="links-item">
			    <div class="links-img"><img src="'.Utils::addLoadingImages($GLOBALS['miraclesOptions_CDN'] ?? '0', $GLOBALS['miraclesOptions_loading_image'] ?? 'block', 'link').'" data-gisrc=\'${3}\'></div>
    ...
    private static function parsePaopaoBiaoqingCallback($match)
    {
        $_cdn = $GLOBALS['miraclesOptions_CDN'] ?? '0';
        $_load = $GLOBALS['miraclesOptions_loading_image'] ?? 'block';
        return '<img class="owo-img" src="'.Utils::addLoadingImages($_cdn, $_load, 'owo').'" data-gisrc="/usr/themes/Miracles/images/biaoqing/paopao/'. str_replace('%', '', urlencode($match[1])) . '_2x.png">';
    }
    ...
    private static function parseAruBiaoqingCallback($match)
    {
        $_cdn = $GLOBALS['miraclesOptions_CDN'] ?? '0';
        $_load = $GLOBALS['miraclesOptions_loading_image'] ?? 'block';
        return '<img class="owo-img" src="'.Utils::addLoadingImages($_cdn, $_load, 'owo').'" data-gisrc="/usr/themes/Miracles/images/biaoqing/aru/'. str_replace('%', '', urlencode($match[1])) . '_2x.png">';
    }
    ...
    private static function parseTweBiaoqingCallback($match)
    {
        $_cdn = $GLOBALS['miraclesOptions_CDN'] ?? '0';
        $_load = $GLOBALS['miraclesOptions_loading_image'] ?? 'block';
        return '<img class="owo-img" src="'.Utils::addLoadingImages($_cdn, $_load, 'owo').'" data-gisrc="/usr/themes/Miracles/images/biaoqing/twemoji/'. str_replace('%', '', $match[1]) . '.png">';
    }
    ...
    {
        $id = trim($matchs[3]);
        $_anchor = $GLOBALS['postTexts']['post_anchor'] ?? '锚点';
        return '<h'.$matchs[1].$matchs[2].' id="'.$id.'">'.$matchs[3].'<a href="#'.$id.'" title="'.$_anchor.'" class="post-toc-link no-line"><i class="iconfont icon-paragraph"></i></a></h'.$matchs[1].'>';
    }
